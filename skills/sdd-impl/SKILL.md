...
3. **Recipe present?** Confirm `./tech-stack.md` exists in cwd. If missing, say in Russian: «В проекте нет `tech-stack.md` — рецепт стека, по которому я собираю фазы. Похоже, план создан старой версией SDD. Запусти `/sdd-idea` с опцией «Начать заново» — план и рецепт обновятся вместе.» Then exit.
